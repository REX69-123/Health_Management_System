...
<body class="bg-slate-50 p-6 md:p-12 flex items-center justify-center min-h-screen">
    <div class="w-full max-w-3xl bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
        <div class="bg-amber-500 p-8 text-white">
            ...
            <p class="text-amber-100 text-sm mt-1">Modifying record for: <strong>{{ $record->patient->first_name }} {{ $record->patient->last_name }}</strong></p>
        </div>
        ...
        <form action="{{ route('medical-records.update', $record->id) }}" method="POST" class="p-10 space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div>
                    <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2">Patient (Read-Only)</label>
                    <input type="text" disabled value="{{ $record->patient->first_name }} {{ $record->patient->last_name }}"
                ...
                <div>
                    ...
                    <input type="date" name="diagnosis_date" value="{{ old('diagnosis_date', $record->diagnosis_date) }}" required
            ...
            <div class="flex items-center justify-between pt-8 border-t border-slate-100">
                <a href="{{ route('medical-records.index') }}" class="text-slate-400 font-bold hover:text-slate-600 transition-colors">Cancel Changes</a>
                <button type="submit" class="bg-slate-800 hover:bg-slate-900 text-white font-bold px-10 py-3 rounded-xl transition-all shadow-lg active:scale-95">
