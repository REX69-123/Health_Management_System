...
<body class="bg-slate-50 min-h-screen p-8 flex items-center justify-center">
    ...
    <div class="w-full max-w-2xl bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
        <div class="px-8 py-6 border-b border-slate-200 flex justify-between items-center">
            ...
            <a href="{{ route('appointments.index') }}" class="text-sm text-slate-400 hover:text-slate-600">Back to List</a>
        </div>
        ...
        <form action="{{ route('appointments.store') }}" method="POST" class="p-8 space-y-6">
            ...
            <div>
                ...
                <select name="patient_id" required
                    class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-600 outline-none appearance-none bg-white">
            ...
            <div class="flex justify-end pt-4">
                <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold px-8 py-2.5 rounded-lg transition-colors shadow-sm">
                    Confirm Appointment
                </button>
            </div>
        </form>
